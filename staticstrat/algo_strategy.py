...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        self.planner = []
        #C:\\Users\\TheDonut\\Documents\\Berkeley\\terminal\\C1GamesStarterKit\\staticstrat\\
        with open("C:\\Users\\TheDonut\\Documents\\Berkeley\\terminal\\C1GamesStarterKit\\staticstrat\\config.json") as f:
    ...
    def build_defences_min(self, game_state):
        """
        ...
        # destructor_locations = [[0, 13], [27, 13], [13, 11], [10, 8], [13, 5], [16, 8]]
        destructor_locations = [[i, 13] for i in [2, 3, 10, 17, 24, 25]]
        game_state.attempt_spawn(DESTRUCTOR, destructor_locations)
        if game_state.turn_number >= 4:
            destructor_locations = [[12, 6], [12, 9], [15, 9], [2, 12], [15, 6], [3, 10], [6, 8]]
            game_state.attempt_spawn(DESTRUCTOR, destructor_locations)
            filter_locations = [[i, 11] for i in range(5, 26)] + [[2, 11], [3, 11]]
            game_state.attempt_spawn(FILTER, filter_locations)
            encryptor_locations = [[i, 8] for i in range(20, 23)] + [[24, 10]]
    ...
    def build_reactive_defense(self, game_state):
        ...
        for location in self.scored_on_locations:
            ...
            if location[1] > 11:
                if location[0] >= 14:
                    build_locations = [[location[0] - i, location[1] + j] for i in range(5) for j in range(-2, 3)]
                else:
                    build_locations = [[location[0] + i, location[1] + j] for i in range(5) for j in range(-2, 3)]
            for build_location in build_locations:
                if not game_state.contains_stationary_unit(location):
                    game_state.attempt_spawn(DESTRUCTOR, build_location)
        for loc in [[11, 10], [16, 10],[11, 11], [16, 11]]:
