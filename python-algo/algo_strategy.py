...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def my_strategy(self, game_state):
        ...
        self.madrox_copy_defences(game_state)
        if game_state.turn_number == 1:
            game_state.attempt_spawn(PING, [13, 0], 8)
        if game_state.turn_number >= 4:
            game_state.attempt_spawn(EMP, [23, 9], 10)

    def madrox_copy_defences(self, game_state):
        """
        ...
        destructor_locations = [[i, 13] for i in [2, 3, 24, 25]]
        if game_state.turn_number < 4:
            game_state.attempt_spawn(DESTRUCTOR, [[10, 17], [13, 17]])
        game_state.attempt_spawn(DESTRUCTOR, destructor_locations)
        if game_state.turn_number >= 4:
            destructor_locations = [[12, 6], [12, 9], [15, 9], [2, 12], [15, 6], [3, 10], [6, 8]]
            game_state.attempt_spawn(DESTRUCTOR, destructor_locations)
            if game_state.turn_number >= 5:
                game_state.attempt_spawn(DESTRUCTOR, [[24, 11], [25, 11]])
            filter_locations = [[26 - i, 11] for i in range(22)] + [[2, 11], [3, 11]]
